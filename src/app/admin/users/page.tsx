...
export default function Users() {
  ...
  const getData = async () => {
    try {
      ...
      let filter5 = status ? `&status=${status}` : "";
      let filter6 = `&type=editor`;
      let URL = `${API.USERS}${filter1}${filter2}${filter3}${filter4}${filter5}${filter6}`;
  ...
  const deleteItem = async (item: any) => {
    try {
      let response: any = await DELETE(`${API.USERS}?id=${item?._id}`);
      if (response?.status) {
        getData();
    ...
    <div>
      ...
      >
        <Button type="primary" onClick={() => setFormModal(true)}>
          <span className="text-[12px] hidden md:inline">New user</span>
          <IoMdAdd size={20} />
        </Button>
