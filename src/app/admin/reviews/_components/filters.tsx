...
const Filters = (props: any) => {
  ...
  return (
    <Form
      form={form}
      initialValues={{ query: props?.query, status: props?.status }}
      onValuesChange={(_v, values) => onValuesChange(values)}
    >
        ...
        <Form.Item noStyle name={"status"}>
          <Segmented<string>
            options={[
              ...
              { label: "Disabled", value: "blocked" },
            ]}
          />
        </Form.Item>
        <Button type="primary" onClick={() => props?.openForm()} className="px-3!">
          <span className="hidden sm:block!">New Review</span>
          <IoMdAdd size={20} />
        </Button>
      </div>
    </Form>
